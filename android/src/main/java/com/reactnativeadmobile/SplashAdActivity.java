...
public class SplashAdActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 动态获取底部图片高度
        int bottomImageHeightPx = getBottomImageHeightPx(widthPixels);
...
//        Log.e(TAG, "调整后图片高度: " + adjustedBottomImageHeightPx + "px");
        Log.e(TAG, "状态栏高度: " + statusBarHeight + "px");
        Log.e(TAG, "广告高度: " + adHeight + "px");
        Log.e(TAG, "屏幕总高度: " + heightPixels + "px");
//        Log.e(TAG, "调整像素数: " + (bottomImageHeightPx - adjustedBottomImageHeightPx) + "px");

        // 创建额外参数实例
        ADSuyiExtraParams extraParams = new ADSuyiExtraParams.Builder()
                // 设置整个广告视图预期宽高(目前仅头条平台需要，没有接入头条可不设置)，单位为px，如果不设置头条开屏广告视图将会以9 : 16的比例进行填充，小屏幕手机可能会出现素材被压缩的情况
                .adSize(new ADSuyiAdSize((int) widthPixels, adHeight))
                .build();
        // 如果开屏容器不是全屏可以设置额外参数
...
//        }, 500); // 500ms后再次检查
    ...
    private int getStatusBarHeight() {
        try {
            ...
            if (resourceId > 0) {
                ...
                // 如果无法获取状态栏高度，使用默认值
                int defaultHeight = (int) (24 * getResources().getDisplayMetrics().density);
                Log.w(TAG, "无法获取状态栏高度，使用默认值: " + defaultHeight + "px");
                return defaultHeight;
            }
        } catch (Exception e) {
            Log.e(TAG, "获取状态栏高度失败: " + e.getMessage());
            // 发生异常时使用默认值
            int defaultHeight = (int) (24 * getResources().getDisplayMetrics().density);
            return defaultHeight;
        }
     ...
     * @return 底部图片高度（像素）
     */
    private int getBottomImageHeightPx(float screenWidth) {
        try {
            // 获取屏幕密度
            final float density = getResources().getDisplayMetrics().density;
...
//            int screenWidth = getResources().getDisplayMetrics().widthPixels;
            ...
            options.inDensity = 0; // 不进行密度缩放，获取原始尺寸
            options.inTargetDensity = 0; // 不进行密度缩放
